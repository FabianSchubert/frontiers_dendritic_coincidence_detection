...
def run_sim(arglist):
    ...
    for t in tqdm(range(1,T),disable=True,leave=False):
        ...
        if(mode=="point"):
            w_p[t] = w_p[t-1] + mu_w * np.outer(y[t-1]*(y[t-1]-y_squ_av[t-1])/y_squ_av[t-1],x_p[t-1] - x_p_av[t-1])
    ...
    return (1.*(pred == lab_test)).mean()

pool = Pool()
perf_list = list(tqdm(pool.imap_unordered(run_sim, params), total=len(params)))
...
for mode in modes:
    for s in range(N_sweep_distraction_scaling):
        for n in range(N_sweep_distraction_dimension):
            for i in range(N_samples):
                perf[s,n,i,modes.index(mode)] = perf_list[k]
                k += 1
...
np.savez(os.path.join(DATA_DIR,"classification_scaling_dimension_bcm.npz"),
         perf = perf,
