...
N_sweep_distraction = 9
N_samples = 2
distract_scaling = 2.
distract_dimension = np.arange(1,N_sweep_distraction+1)
...
for mode in tqdm(modes):
    for s in tqdm(range(N_sweep_distraction),leave=False):
        for i in tqdm(range(N_samples),leave=False):
            ...
            x_p[:,1:distract_dimension[s]+1] *= distract_scaling
            x_p = (Q @ x_p.T).T
            ...
            x_p_test[:,1:distract_dimension[s]+1] *= distract_scaling
...
np.savez(os.path.join(DATA_DIR,"distraction_dimension.npz"),
